...
#		await m.add_reaction("👎")
		...
		for m in ctx.guild.members:
			if (m.nick and m.nick.startswith("!")) or m.name.startswith("!"):
				...
				if m.top_role >= ctx.author.top_role:
					...
					await m.edit(nick=f"{newnick}".replace("!", ""), reason=f"Dehoisted by {ctx.author} ({ctx.author.id})")
	...
	async def on_raw_reaction_add(self, payload):
		...
		if str(payload.emoji) == "📣" and payload.user_id != self.bot.user.id and not (await bot.fetch_user(payload.user_id)).bot:
